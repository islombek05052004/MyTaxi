...
class AddPromocodViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            self.mainView.promocodTF.becomeFirstResponder()
        }
    }
    ...
    private func setTargetFunctions() {
        self.mainView.addBtn.addTarget(self,
                                       action: #selector(addPromocodButtonTapped),
                                       for: .touchUpInside)
    }
    ...
    @objc private func addPromocodButtonTapped() {
        ...
        if let promokod = mainView.promocodTF.text {
            self.presenter.checkPromocod(promocod: promokod)
        }
    }
}
